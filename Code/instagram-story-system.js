...
Webflow.push(function() {
  ...
  // Instagram Story System with proper timer management
  var Webflow = Webflow || [];
  Webflow.push(function() {
    let autoAdvanceTimer = null; // Single timer variable
    ...
    function startAutoAdvance() {
      ...
      autoAdvanceTimer = setInterval(nextTab, 5000);
    }
    ...
    function nextTab() {
      if (!$(".uui-navbar06_menu-button").hasClass("w--open") && !isNavigating) {
          ...
          if (window.craftMenu && typeof window.craftMenu.previousCategory === 'function') {
            ...
            setTimeout(() => {
              ...
              if (visibleTablinks.length > 0) {
                ...
                lastTab[0].click(); // Use native click to avoid jQuery event bubbling
                console.log(`🎯 Navigated to last tab (${lastTabIndex})`);
              }
              isNavigating = false;
              startAutoAdvance(); // Restart timer after navigation
            }, 200);
          } else {
          ...
          if (window.craftMenu && typeof window.craftMenu.nextCategory === 'function') {
            ...
            setTimeout(() => {
              ...
              if (visibleTablinks.length > 0) {
                ...
                firstTab[0].click(); // Use native click to avoid jQuery event bubbling
  ...
  // Check visible tabs
  const visibleTabs = document.querySelectorAll('.fs-tabs.tabs-visible');
  console.log('Visible tabs:', {
    count: visibleTabs.length,
    categories: Array.from(visibleTabs).map(tab => tab.getAttribute('data-category'))
  });
  
  // Check buttons
  const allButtons = document.querySelectorAll('.tab_previous, .tab_next');
  const visibleButtons = document.querySelectorAll('.fs-tabs.tabs-visible .tab_previous, .fs-tabs.tabs-visible .tab_next');
  ...
  window.debugTabSystem = function() {
    console.group('🔍 Tab System Debug');
    console.log('Auto-advance timer:', autoAdvanceTimer);
    console.log('Is navigating:', isNavigating);
    console.log('Menu open:', $(".uui-navbar06_menu-button").hasClass("w--open"));
    console.log('Visible tabs:', $('.fs-tabs.tabs-visible').length);
    console.groupEnd();
  };
});
